...
const app = new Hono()
  ...
  .get(
    ...
    async (c) => {
      ...
      const res = await fetch(url);
      if (!res.ok) return c.json({ error: "Lyrics not found" }, 404);
      const data = await res.json();
      return c.json({ lyrics: data.lyrics as string });
    },
  )
  .post(
    "/export",
    ...
    async (c) => {
      const { deckName, cards } = c.req.valid("json");
